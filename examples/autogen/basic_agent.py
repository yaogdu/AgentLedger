...
from agentledger import AutoGenAdapter, Runtime


class FakeAutoGenAgent:
    def generate_reply(self, payload: dict[str, str]) -> dict[str, str]:
        return {"reply": f"autogen-style agent handled {payload['topic']}"}


async def main() -> None:
    with tempfile.TemporaryDirectory() as tmp:
        rt = Runtime.local(Path(tmp) / ".agentledger")
        adapter = AutoGenAdapter(FakeAutoGenAgent(), input_mapper=lambda _ctx, state: {"topic": state["topic"]})
