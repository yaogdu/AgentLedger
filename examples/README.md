...
| OMP runtime bridge | `omp_bridge/README.md` | Maps synthetic Oh My Pi / OMP runtime records into AgentLedger evidence without OMP internals or application-specific semantics. Cross-language commands are listed below. |
...
| OMP runtime bridge | `PYTHONPATH=src python3 examples/omp_bridge/demo.py` | `cd go && go run ./examples/omp_bridge` | `cd typescript && node examples/omp_bridge/omp_bridge.js` | `cd rust && cargo run --example omp_bridge` |
