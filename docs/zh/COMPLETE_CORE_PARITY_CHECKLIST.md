...
- 跨语言通用 official adapter API：Postgres、S3/MinIO、OTLP transport、Docker sandbox manifest 和 command-style Docker execution
...
- 应用管理后台、full eval system、workflow engine、RAG system、sandbox provider
...
4. Python、TypeScript、Rust 的 package release family 已自动检查；当前 core release family 是 `1.3`。Packaged Python/Inspector patch release 可以通过 Python/PyPI 发布，例如 `1.3.6`，不代表非 Python runtime-core baseline 必须同时变化。
