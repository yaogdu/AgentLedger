...
注意：`go get` 必须在 Go module 内运行。`go install github.com/yaogdu/AgentLedger/go@v1.4.1` 会失败，因为 library package 不是 `package main`；安装 CLI 要使用 `/cmd/agentledger-go` 路径。
