...
|---|---|---|---|
| Storage | `StateStoreProtocol`、migration、lease/fencing invariants | SQLite WAL + local blob store | Postgres、MySQL、S3/MinIO、自定义 store |
...
| 层 | 接口 | 常见实现 |
|---|---|---|
| State | `StateStore` | SQLite WAL, Postgres, MySQL, custom store |
