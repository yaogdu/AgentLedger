...
| 优先级 | 事项 | 验收标准 |
|---|---|---|
| P0 | 传播型 side-effect showcase | 用户可以运行 `examples/showcase/duplicate_side_effect_crash`，看到 naive retry 的重复副作用、AgentLedger 的安全恢复，以及生成的 Inspector HTML。 |
...
| P0 | 传播文章和短帖 | 维护者可以直接使用 `docs/articles/what-happened-after-agent-tool-failure.zh-CN.md` 和 `docs/articles/social-posts.md`，不用临时重新组织项目定位。 |
...
| P1 | OpenAI Agents SDK-style approval/replay example | dependency-free SDK-style runner 展示 model evidence、proposed tool call、approval、Tool Ledger、evidence export、Inspector 和 replay。 |
| P1 | Temporal bridge retry safety example | dependency-free workflow/activity facade 展示 backend retry，同时 AgentLedger 阻止 node 内部重复副作用。 |
...
| 标题 | 类型 | 为什么重要 |
|---|---|---|
| Record a side-effect safety GIF from the showcase | adoption | 把核心价值变成 README、社交发布和 issue discussion 可使用的短视觉材料。 |
| Build a cross-language 3-minute Tool Ledger demo | example | 几分钟内跨 Python、Go、TypeScript、Rust 展示核心价值。 |
| Add OpenAI Agents SDK approval/replay example | example | 对接主流 agent SDK 边界，但不声明官方 endorsement。已作为 `examples/openai_agents/approval_replay.py` 实现。 |
...
| Add Temporal bridge example | integration | 说明 Temporal 管 workflow lifecycle，AgentLedger 管 node 内部 reliability。已作为 `examples/temporal_bridge/demo.py` 实现。 |
