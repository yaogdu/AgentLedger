...
```bash
PYTHONPATH=src python3 -m agentledger adapter certify --kind postgres --adapter-version 1.2.1
PYTHONPATH=src python3 -m agentledger adapter certify --kind s3 --adapter-version 1.2.1 --out ./s3-certification.json
PYTHONPATH=src python3 -m agentledger adapter certify --kind langgraph --adapter-version 1.2.1 --package-name agentledger-langgraph
```
