...
| TypeScript/Node | `agentledger-runtime` subpath exports，加 `typescript/packages/` 下 npm adapter package skeleton | subpath exports 方便本地使用；独立 package skeleton 保留后续独立发布能力 |
| Go | `go/adapters/...` 下可 import 的 adapter subpackages | Go 没有 extras，用户一般消费同一个 module 下的 subpackage |
| Rust | crate features，加 `rust/crates/` 下 adapter crate skeleton | Rust 可以通过 feature-gated boundary 或独立 crate 接入 |
...
| `agentledger-langgraph` | 基于 dependency-free facade 的 LangGraph checkpointer/node wrapper | `langgraph` |
| `agentledger-mcp` | 围绕 `ToolSpec` 和 context read 的真实 MCP client/server transport adapters | MCP SDK |
| `agentledger-otel` | 围绕 AgentLedger spans 的 OpenTelemetry SDK/exporter 集成 | OpenTelemetry packages |
| `agentledger-sandbox-docker` | Docker sandbox executor package 和本地/团队 recipe | Docker SDK 或 Docker CLI |
