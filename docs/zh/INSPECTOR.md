...
`1.4.2` 为 Runtime Model Evidence Boundary 增加专门的 Inspector 区块：

- `Model Calls`：展示 `model_call_requested`、`model_call_completed`、`model_call_failed` evidence，包括 provider/model metadata、request/response/failure refs、usage、cost、step id 和 status。
- `Tool Proposals`：展示 ToolGateway 执行前的 `tool_call_proposed` records，包括模型建议的 tool name、arguments refs、model/provider metadata、confidence、reason，以及可用时到 model-call evidence 的链接。

这些区块用于回答排查问题：

- 这个 run 当时绑定的完整 model request / response 是什么？
- model 是否在 runtime 执行工具前提出了 tool call？
- failure 是 model call、runtime，还是 tool execution 导致的？
- 哪些 evidence refs 应该导出给 Langfuse、OpenTelemetry、LangSmith、Temporal-style consumers 或 CI？

Inspector 仍然不做 model routing、provider retry、key management、pricing catalog，也不会访问 model provider。它只读取 runtime、用户代码、framework adapter、provider SDK 或外部 model gateway 已经记录进 AgentLedger 的 evidence。
