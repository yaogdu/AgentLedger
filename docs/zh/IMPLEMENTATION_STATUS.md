...
AgentLedger 1.0.x 是 stable runtime-core line，Python 是 reference implementation，Go/TypeScript/Rust 已通过共享 runtime-core parity gate 覆盖；1.0.5 development line 增加 normalized Policy Engine decision contract，同时继续明确 concrete adapter path 中的 preview/experimental 边界。它适合：
...
对当前 1.0.x 目标来说，“stable runtime-core”指 Python reference runtime 已经可用、已文档化、已测试、可按 release gate 检查、contract 已冻结，并且 Go/TypeScript/Rust 已有 runtime-core parity gate。它不代表所有 optional production adapter 或外部 eval integration 已经在每种语言都发布。
...
| Evidence regression primitives | side-effect-free evidence checks、`evidence-regression` media/stream gates、adversarial review, evidence regression checklist、divergence report、golden corpus seed/add/list/check |
