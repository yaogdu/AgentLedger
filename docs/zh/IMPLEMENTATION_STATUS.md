...
AgentLedger 1.0.1 是 stable runtime-core release，Python 是 reference implementation，Go/TypeScript/Rust 已通过共享 runtime-core parity gate 覆盖；部分 concrete adapter path 仍保持 preview/experimental。它适合：
...
对当前 1.0.1 目标来说，“stable runtime-core”指 Python reference runtime 已经可用、已文档化、已测试、可按 release gate 检查、contract 已冻结，并且 Go/TypeScript/Rust 已有 runtime-core parity gate。它不代表所有 optional production adapter 或外部 eval integration 已经在每种语言都发布。
