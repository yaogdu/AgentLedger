...
AgentLedger 1.4.x 是 stable runtime-core line，Python 是 reference implementation，Go/TypeScript/Rust 已通过共享 runtime-core parity gate 覆盖。1.4 line 保持 stable runtime-core contract，同时增加 language-neutral、read-only Inspector companion path，并在四种 runtime package 中完成 Agent Failure Lifecycle baseline。它适合：
