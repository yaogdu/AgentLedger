...
AgentLedger 1.5.x 是 stable runtime-core line，Python 是 reference implementation，Go/TypeScript/Rust 已通过共享 runtime-core parity gate 覆盖。1.5 line 保留 1.4 已稳定的 runtime-core contract，继续保留 language-neutral、read-only Inspector companion path，并在不改变 portable runtime-core semantics 的前提下补充首批 framework/Temporal adoption examples 和 smoke coverage。它适合：
...
版本范围说明：1.3.x 增加并完善 Inspector，定位是只读 evidence/runtime metadata consumer。它可以读取导出的 evidence bundle，也可以用只读凭证连接 SQLite/Postgres/MySQL AgentLedger metadata，并导出静态 HTML debug report。1.4.0 增加四语言 Agent Failure Lifecycle baseline：normalized failure envelopes、lifecycle stages、causal graph、replay plan、regression report、本地 alert records 和 portable failure export mappings。1.4.1 增加四语言 Runtime Model Evidence Boundary：archived model request/response/failure evidence、model-proposed tool call、model cost/failure/replay semantics，但不做 provider routing。1.4.2 在这条路径上做收敛：增加专门的 Inspector model-call/tool-proposal 视图、更强的 failure export mappings、model evidence example 和 boundary lint 加固。1.5.0 保持这套 runtime-core contract 不变，同时新增 dependency-free OpenAI Agents SDK-style approval/replay example、Temporal bridge example、framework-native smoke coverage、benchmark gate 和对应文档入口。1.5.2 在 Python、Go、TypeScript、Rust 中增加 normalized Oh My Pi / OMP runtime bridge：把 session、turn、model-call、tool-proposal、tool-execution、failure、artifact 和 versioned state-change record 映射到现有 AgentLedger evidence，同时不引入应用专属语义。MySQL 仍是 1.2.2 的 storage adapter boundary，Langfuse 仍是 1.2.3 的 observability adapter boundary；真实服务生产级声明仍需要外部验证。
...
release gates 证明的是已覆盖的 runtime contract 和 conformance 场景，不是证明项目不存在任何缺陷。当前自动化检查覆盖 unit tests、dependency-free examples、contract snapshots、adapter package metadata、跨语言 runtime-core conformance、parity audits、lint rules 以及 Markdown/link hygiene。

以下内容在做生产级声明前仍需要单独验证：

- 生成的静态 HTML report 在真实浏览器中的视觉回归
- 真实 Postgres/MySQL/S3/OTLP/sandbox 服务在实际并发和失败模式下的行为
- framework-native adapters 与真实框架版本的兼容性
- 组织自己的 policy pack、secret handling、redaction rule 和 approval workflow
- 大 evidence bundle、长运行 run、高基数生产数据
- 业务项目集成 smoke test，例如 `legal-agent`

本文说某个 feature 已实现或 stable，含义是它已经存在，并且被当前声明的 contract/test boundary 覆盖。它不表示所有下游集成、浏览器渲染边界或外部基础设施路径都已被穷尽验证。

## 当前完成边界

对当前 1.5.x 目标来说，“stable runtime-core plus adoption proof around framework/Temporal boundaries”指 Python reference runtime 已经可用、已文档化、已测试、可按 release gate 检查、contract 已冻结，并且 Go/TypeScript/Rust 已由 runtime-core parity gate 覆盖。它也表示第一方 adapter boundary 已在适合的语言生态中完成 package/import 边界，Inspector 可以消费跨语言 evidence，并且仓库里已经有 dependency-free example 展示 framework-owned execution 如何仍然记录 runtime-managed approval、Tool Ledger、model evidence、replay 和 retry safety。它不代表所有 optional production adapter 或外部 eval integration 都已在每种语言生产加固。
...
| Cost 与 budget | store-backed cost record、budget enforcement hook、按 run/agent/step/category/tool/model 的只读 cost attribution report，以及 model evidence usage/USD attribution |
| Runtime Model Evidence Boundary | `agentledger.model.evidence.v1`、external model-call evidence API、`model_call_requested/completed/failed`、`tool_call_proposed`、model failure category、replay-safe archived model evidence semantics、Inspector model-call/tool-proposal 视图，以及 model evidence refs 的 export mappings |
...
| Adapter contract | framework adapter base、LangGraph facade、MCP tool/context mapping、dependency-free method facades、normalized OMP runtime bridge、第一方 adapter package boundaries |
...
| Reliability checks | failure injection suite、failure attribution report、conformance runner、runtime-boundary lint、scheduler facade、adversarial review、针对 shell、HTTP、database、filesystem、cloud、GitHub、risky ToolSpec metadata 和常见 model SDK bypass 的 evidence regression、JSON rule-pack extension |
