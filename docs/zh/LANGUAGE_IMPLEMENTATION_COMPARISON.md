...
Patch 版本不需要在所有语言里强行同步。当某个 release 只修改某个语言的 package、companion tool、read model 或 adapter wrapper 时，只发布受影响的包即可。`1.4.0` 和 `1.4.1` 不同：它们分别是 Agent Failure Lifecycle 和 Runtime Model Evidence Boundary 的四语言 runtime-core feature release，所以 Python、Go、TypeScript、Rust 都进入同一个 runtime-core release family。Inspector 仍是通过 Python/PyPI 分发的 language-neutral companion viewer，但 failure lifecycle 和 model evidence contract 本身已经在四种 runtime package 中实现。
...
| Model evidence boundary | Yes | Yes | Yes | Yes | `agentledger.model.evidence.v1`、`model_call_requested/completed/failed` 和 `tool_call_proposed`；core 不做 provider routing。 |
