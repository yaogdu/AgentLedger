...
| OMP runtime bridge | Yes | Yes | Yes | Yes | normalized session/turn/model/tool/failure/state records 映射到 AgentLedger evidence，不依赖 OMP 或应用专属实现。 |
