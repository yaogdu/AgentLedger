...
| Model providers/router | OpenAI、Anthropic、Gemini、Bedrock、Azure OpenAI、Ollama、LiteLLM-style bridge | Model call 会影响 evidence、cost、replay、fallback 和 budget enforcement，但 provider SDK 不应成为 runtime-core 依赖。 | 等 `ModelGateway`/`ModelRouter` contract 稳定后再稳定 provider packages。 |
...
| Model provider/router | ModelGateway contract 之后推荐 | ModelGateway contract 之后推荐 | ModelGateway contract 之后推荐 | ModelGateway contract 之后推荐 | provider SDK 保持 optional；使用 injected client 和 conformance fixtures。 |
