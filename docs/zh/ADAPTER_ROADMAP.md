...
- adapter 会迫使 core 变成 workflow engine、eval system、应用管理后台或 deployment product。
...
| --- | --- | --- | --- | --- |
| Storage | Postgres / MySQL StateStore | 企业 pilot 通常需要服务端 durable state、locking、migration 和 backup workflow。 | `storage_schema.v1.json`、local persistence semantics、`optional_adapters.v1.json` | `agentledger-postgres`、`agentledger-mysql` / 各语言原生 package |
...
| Runtime | Oh My Pi / OMP | OMP 是一个 runtime 边界，可以把 session、turn、model、tool、failure、artifact 和 state-change evidence 写入 AgentLedger，同时不让 AgentLedger 变成应用专属实现。 | 1.5.2 已在 Python、Go、TypeScript、Rust 内置 normalized bridge；只有当 bridge 超出薄翻译层后，再考虑打包为 `agentledger-omp`。 |
...
| Deployment | Kubernetes worker recipe | 对 pilot 有用。 | 先做 recipe/Helm/example，部署管理能力保持在 runtime-core 外。 |
| Model evidence endpoints | OpenAI-compatible endpoint、Anthropic-style call、企业 gateway | Model call 会影响 evidence、cost、replay 和 failure attribution，但 provider SDK 和 router 应保持在 runtime-core 外。 | 使用 Runtime Model Evidence Boundary；除非边界保持 evidence-only，否则不做官方 router/gateway adapter。 |
...
| Eval | Langfuse、Phoenix、promptfoo、DeepEval、Ragas、OpenAI Evals、LangSmith、Braintrust、自定义 CI | Eval 是 evidence/replay contract 的消费者，不属于 runtime-core。AgentLedger 应提供 adapter/export format，而不是 standalone Eval Platform。 |
...
| Model evidence boundary | core 必做 | core 必做 | core 必做 | core 必做 | 记录外部 model call、failure、cost 和 tool proposal，但不打包 provider SDK 或 router。 |
...
6. 继续加固内置 Oh My Pi / OMP runtime bridge，补 redaction/evidence-completeness fixtures；只有当独立 package 边界有价值时再创建 optional package。
7. 在生态稳定处补 LangChain / CrewAI / AutoGen / OpenAI Agents SDK / LlamaIndex / Semantic Kernel facade。
8. Kubernetes sandbox/backend recipe，然后再做 optional execution adapter。
9. 根据真实用户需求做 Temporal/Ray/Kubernetes scheduler/backend bridge。
10. 增加 OpenAI-compatible endpoint、Anthropic-style call、企业 gateway 的 model evidence examples，但不把 AgentLedger 做成 model router。
11. 仅评估，不承诺实现：Routing Advisor / Capability Router evidence boundary 是否对 WisePick-style services 有价值。如果后续采纳，应记录外部传入的 route suggestion，replay 时复用已存 routing evidence，并且只在新的真实执行完成后发送 feedback。
12. 只在边界保持 evidence/replay conversion 时，为成熟开源 eval 工具增加 eval adapter/export packages：Langfuse datasets/scores、Phoenix experiments、promptfoo test cases、DeepEval cases、Ragas rows、OpenAI Evals samples 和 CI report inputs。不要在 AgentLedger 里做 dataset/scorer/experiment 管理。
