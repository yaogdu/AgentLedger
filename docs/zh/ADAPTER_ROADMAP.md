...
9. 增加 OpenAI-compatible endpoint、Anthropic-style call、企业 gateway 的 model evidence examples，但不把 AgentLedger 做成 model router。
...
11. 只在边界保持 evidence/replay conversion 时，为成熟开源 eval 工具增加 eval adapter/export packages：Langfuse datasets/scores、Phoenix experiments、promptfoo test cases、DeepEval cases、Ragas rows、OpenAI Evals samples 和 CI report inputs。不要在 AgentLedger 里做 dataset/scorer/experiment 管理。
