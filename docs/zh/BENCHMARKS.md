...
GitHub Actions 也会在 `main` push 和 pull request 上执行完整 benchmark，对应 job 名为 `Runtime benchmark gate`。该 job 会上传名为 `agentledger-benchmark` 的 artifact，里面包含 `benchmark.json`、`benchmark.md` 和隔离的 `run-*/` evidence/debug artifacts。发布时可以把最新成功 CI artifact 作为 release evidence；Postgres、MySQL、S3、Docker、Langfuse、OTLP collector、Temporal 或其它 production adapter 声明仍需要单独的真实服务验证。
