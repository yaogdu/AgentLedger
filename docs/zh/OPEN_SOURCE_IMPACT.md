...
AgentLedger 是一个早期开源基础设施项目，面向生产级 AI Agent 的可靠性与治理问题。它不是 Agent 框架，不是应用管理后台，也不替代 LangGraph、Temporal、Langfuse、MCP 或模型供应商。它的价值在于提供这些系统可以共享的 runtime boundary：durable state、governed tool use、evidence、replay、policy check、sandbox routing 和 cost/failure attribution。
