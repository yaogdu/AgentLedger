...
AgentLedger 1.3.0 has one Python reference runtime and native preview runtime-core baselines for Go, TypeScript, and Rust. The shared promise is semantic parity for runtime-core: durable runs, leases, Tool Ledger, evidence, replay, policy/approval/sandbox boundaries, cost/failure attribution, conformance, and official optional adapter contracts.
...
Note: `go get` must run inside a Go module. `go install github.com/yaogdu/AgentLedger/go@v1.3.0` fails because the library package is not a `package main`; use `/cmd/agentledger-go` for the CLI.
...
- Official adapters in 1.3.0 use injected clients and dry-run manifests; real cloud SDKs and live service hardening are optional follow-up gates.
