...
AgentLedger 1.3.x has one Python reference runtime and native preview runtime-core baselines for Go, TypeScript, and Rust. The shared promise is semantic parity for runtime-core: durable runs, leases, Tool Ledger, evidence, replay, policy/approval/sandbox boundaries, cost/failure attribution, conformance, and official optional adapter contracts. Packaged Inspector/failure-read-model patch releases such as 1.3.6 may publish through Python/PyPI while the non-Python runtime-core package baselines remain on the latest shared runtime-core tag. That means the official viewer/CLI distribution changed; it does not mean Go, TypeScript, or Rust lost runtime-core failure support.
