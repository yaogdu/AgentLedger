...
Note: `go get` must run inside a Go module. `go install github.com/yaogdu/AgentLedger/go@v1.5.2` fails because the library package is not a `package main`; use `/cmd/agentledger-go` for the CLI.
...
## Cross-Language OMP Bridge Demo

The normalized OMP bridge is implemented in all four runtime packages. These demos use synthetic OMP-facing records, not OMP internals or application-specific paths:
