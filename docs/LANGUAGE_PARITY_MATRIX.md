...
This matrix tracks the target parity across AgentLedger language implementations. Python is the current v1.0 reference runtime. Go, TypeScript, and Rust are native runtime implementations; SDK/client-only milestones may appear first, but they do not count as runtime parity.

For the concrete side-by-side table of runtime-core, portable adapters, provider-specific differences, Python-only ecosystem adapters, and directory-layout decisions, see `LANGUAGE_IMPLEMENTATION_COMPARISON.md`.
