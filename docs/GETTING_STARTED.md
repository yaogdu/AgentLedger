...
| Go | `go get github.com/yaogdu/AgentLedger/go@v1.1.0` inside a Go module | `cd go && go run ./examples/quickstart` from this repo | `../go/examples/README.md` | Go module `github.com/yaogdu/AgentLedger/go`, CLI package `.../go/cmd/agentledger-go` |
...
Important: `go get` must run inside a Go module. `go install github.com/yaogdu/AgentLedger/go@v1.1.0` is not valid because that path is a library, not `package main`. Use `/cmd/agentledger-go` for the CLI.
