...
Important: `go get` must run inside a Go module. `go install github.com/yaogdu/AgentLedger/go@v1.2.0` is not valid because that path is a library, not `package main`. Use `/cmd/agentledger-go` for the CLI.
