...
| Package install smoke | Install/import/run from published or built package | PyPI `agentledger-runtime==1.3.6` release smoke after publish for current Python/Inspector patch | `go/v1.3.1` clean external `go get` smoke after tag | npm `agentledger-runtime@1.3.1` release smoke after publish | crates.io `agentledger-runtime==1.3.1` release smoke after publish | publish logs + `scripts/check_complete_core_parity.py` dry-run gates | release-gated |
...
4. Package release families are checked across Python, TypeScript, and Rust. The current core release family is `1.3`; packaged Python/Inspector patch releases such as `1.3.6` may publish through Python/PyPI without changing non-Python runtime-core baselines.
