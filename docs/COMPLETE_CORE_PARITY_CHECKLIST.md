...
| Package metadata | Installable package metadata aligned to 1.0.2 | PyPI metadata | `go.mod` | `package.json` | `Cargo.toml` | `scripts/check_complete_core_parity.py` version check | done |
| Package install smoke | Install/import/run from built package | Python wheel built and checked separately before publish | Go external consumption requires Git tag | `npm pack --dry-run` | `cargo package --allow-dirty --no-verify` | `scripts/check_complete_core_parity.py`; Go external smoke is tag-dependent | done |
...
3. TypeScript and Rust package surfaces have package dry-run checks (`npm pack --dry-run`, `cargo package --allow-dirty --no-verify`).
4. Package versions are checked across Python, TypeScript, and Rust.
5. Go external module consumption remains tag/release dependent and should be verified during release, not treated as a runtime-core implementation gap.
