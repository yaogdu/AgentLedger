...
| Package metadata | Installable package metadata in the current 1.4 release family | PyPI metadata `1.4.x` | `go.mod` tag `go/v1.4.1` for current baseline | npm package `1.4.1` for current baseline | `Cargo.toml` `1.4.1` for current baseline | `scripts/check_complete_core_parity.py` release-family check | done |
| Package install smoke | Install/import/run from published or built package | PyPI `agentledger-runtime==1.4.1` release smoke after publish | `go/v1.4.1` clean external `go get` smoke after tag | npm `agentledger-runtime@1.4.1` release smoke after publish | crates.io `agentledger-runtime==1.4.1` release smoke after publish | publish logs + `scripts/check_complete_core_parity.py` dry-run gates | release-gated |
...
4. Package release families are checked across Python, TypeScript, and Rust. The current core release family is `1.4`; 1.4.1 is a four-language runtime-core feature release for Agent Failure Lifecycle plus Runtime Model Evidence Boundary.
