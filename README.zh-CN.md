...
AgentLedger 1.3.x 是 stable runtime-core line，Python 是 reference implementation，Go、TypeScript、Rust 已由共享 runtime-core parity gate 覆盖。当前 Python/Inspector patch 是 1.3.4；Go、TypeScript、Rust runtime-core package baseline 仍是 1.3.1，直到它们进入下一次 runtime-core release。适合本地使用、framework adapter integration、reliability semantics 验证，以及在明确 adapter 边界下做 production pilot 准备。
