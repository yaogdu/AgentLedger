...
最快的价值验证方式是跑 [3 分钟副作用安全 demo](examples/three_minute_demo/README.md)。在这个受控 demo 里，工具副作用已经成功、worker 在提交状态前崩溃、重试通过 AgentLedger 恢复且不会重复外部写入。更具体的生产场景、保证边界和接入判断见 [docs/zh/USE_CASES.md](docs/zh/USE_CASES.md)。
