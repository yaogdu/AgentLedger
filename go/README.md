...
This directory contains the native Go runtime-core baseline for AgentLedger 1.5.2.
...
`go get github.com/yaogdu/AgentLedger/go@v1.5.2` must run inside a Go module. `go install github.com/yaogdu/AgentLedger/go@v1.5.2` is not valid because the library package is not a `package main`; use `/cmd/agentledger-go` for the CLI.
