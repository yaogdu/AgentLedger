...
func run(args []string) error {
	...
	if len(args) == 1 && args[0] == "version" {
		fmt.Println("agentledger-go 1.4.2")
		return nil
	}
	if len(args) == 1 && args[0] == "doctor" {
		fmt.Println(`{"language":"go","version":"1.4.2","status":"ok","runtime_core_parity":true}`)
		return nil
	}
...
func printHelp() {
	fmt.Println(`AgentLedger Go Runtime 1.4.2
...
func validateContract() error {
	...
	text := string(body)
	for _, token := range []string{"\"contract_version\": \"1.0\"", "\"language\": \"go\"", "\"status\": \"preview\"", "media_stream_artifacts.v1.json", "agentledger.model.evidence.v1", "model_call_failed", "tool_call_proposed"} {
