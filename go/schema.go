...
func MigrationsFor(dialect string) ([]Migration, error) {
	...
	if normalized == "mysql" {
		return []Migration{{Version: "0001", Name: "initial_runtime_metadata", Dialect: "mysql", SQL: mysqlInitialDDL}}, nil
	}
...
const schemaMigrationsMySQL = `CREATE TABLE IF NOT EXISTS schema_migrations (
...
const mysqlInitialDDL = `CREATE TABLE IF NOT EXISTS runs (run_id VARCHAR(128) PRIMARY KEY, session_id VARCHAR(128) NOT NULL, status VARCHAR(64) NOT NULL, state_json JSON NOT NULL, state_version BIGINT NOT NULL, created_at DOUBLE NOT NULL, updated_at DOUBLE NOT NULL);
...
CREATE TABLE IF NOT EXISTS artifacts (artifact_id VARCHAR(128) PRIMARY KEY, run_id VARCHAR(128) NOT NULL, step_id VARCHAR(128), name VARCHAR(255), blob_hash VARCHAR(128) NOT NULL, blob_ref TEXT NOT NULL, metadata_json JSON NOT NULL, created_at DOUBLE NOT NULL, INDEX idx_artifacts_run_step (run_id, step_id));`
